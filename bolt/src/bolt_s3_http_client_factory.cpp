...
static const char* HTTP_CLIENT_FACTORY_ALLOCATION_TAG = "BoltHttpClientFactory";
...
std::shared_ptr<HttpClient> BoltS3HttpClientFactory::CreateHttpClient(const Aws::Client::ClientConfiguration& clientConfiguration) const {
  ...
  return Aws::MakeShared<BoltCurlHttpClient>(HTTP_CLIENT_FACTORY_ALLOCATION_TAG, clientConfiguration);
#else
...
void BoltS3HttpClientFactory::InitStaticState() {
  ...
  if (s_InitCleanupCurlFlag) {
    ...
    BoltCurlHttpClient::InitGlobalState();
  }
#if !defined(_WIN32)
...
void BoltS3HttpClientFactory::CleanupStaticState() {
  ...
  if (s_InitCleanupCurlFlag) {
    ...
    BoltCurlHttpClient::CleanupGlobalState();
  }
#endif
}

void SetInitCleanupCurlFlag(bool initCleanupFlag) { s_InitCleanupCurlFlag = initCleanupFlag; }

void SetInstallSigPipeHandlerFlag(bool install) { s_InstallSigPipeHandler = install; }
